...
RUNTIME="${PROJECT_ROOT}/experiment/phase17/protocol/s17_fp12_resource_profile_runtime.py"
...
case "${ACTION}" in
  prepare-all)
    for arm in "${ARMS[@]}"; do
      status_path="${PROJECT_ROOT}/artifacts/phase17/status/s17_fp12_profile_${arm,,}.status.json"
      if [[ -f "${status_path}" ]]; then
        printf '%s already prepared; preserving existing status\n' "${arm}"
      else
        run_one prepare "${arm}"
      fi
    done
  ...
  prepare|launch|inspect)
    ...
    ;;
  *)
    printf 'usage: %s [prepare-all|inspect-all|prepare ARM|launch ARM|inspect ARM]\n' "$0" >&2
    exit 2
    ;;
esac
