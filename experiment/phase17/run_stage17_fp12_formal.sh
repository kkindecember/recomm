...
RUNTIME="${PROJECT_ROOT}/experiment/phase17/protocol/s17_fp12_formal_runtime.py"
...
ARMS=(G0_GRAM_B0_FRESH G1_GRAM_PSID_FULL G2_GRAM_LATTE_FULL N0_NATIVE_PSID N1_NATIVE_LATTE)

case "${ACTION}" in
  prepare-all|inspect-all)
    one_action="${ACTION%-all}"
    for arm in "${ARMS[@]}"; do run_one "${one_action}" "${arm}"; done
    ;;
