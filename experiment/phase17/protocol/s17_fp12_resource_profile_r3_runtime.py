#!/usr/bin/env python3
"""Dynamic five-card, memory-only FP1/FP2 profiles (attempt_003)."""
...
REVISION_ID = "r3_dynamic_five_card_memory_only"
SAFETY_MARGIN_MIB = 4096
RESEARCHER_DIRECTION = (
    "按当前资源给出最快最高效率分配并开始运行；可选择当前最空卡；"
    "所有状态写入 artifacts/phase17/status"
)

PROFILE_SPECS = {
    "G0_GRAM_B0_FRESH": r2.v1.ProfileSpec(
        "G0_GRAM_B0_FRESH", "S17-FP2-PROFILE", 5, 12288, 16384, 2, 1, "gram"
    ),
    "G1_GRAM_PSID_FULL": r2.v1.ProfileSpec(
        "G1_GRAM_PSID_FULL", "S17-FP2-PROFILE", 4, 12288, 16384, 2, 1, "gram"
    ),
    "G2_GRAM_LATTE_FULL": r2.v1.ProfileSpec(
        "G2_GRAM_LATTE_FULL", "S17-FP2-PROFILE", 3, 12288, 16384, 2, 1, "gram"
    ),
    "N0_NATIVE_PSID": r2.v1.ProfileSpec(
        "N0_NATIVE_PSID", "S17-FP1-PROFILE", 2, 4096, 8192, 256, 1, "native"
    ),
    "N1_NATIVE_LATTE": r2.v1.ProfileSpec(
        "N1_NATIVE_LATTE", "S17-FP1-PROFILE", 6, 4096, 8192, 256, 1, "native"
    ),
}


_R2_FROZEN_CONFIG = r2.frozen_config
_R2_TWO_SNAPSHOT_ADMISSION = r2.two_snapshot_admission
...
def experiment_id(arm_id: str) -> str:
    return f"s17_fp12_profile_r3_{arm_slug(arm_id)}"
...
def paths(root: Path, arm_id: str) -> dict[str, Path]:
    ...
    return {
        ...
        "log": result / "run.log",
        "matrix": root / r2.v1.MATRIX_SUFFIX,
        "allocation": root / r2.v1.ALLOCATION_SUFFIX,
        "tokenizer_status": root / r2.v1.TOKENIZER_STATUS_SUFFIX,
        "vocab_manifest": root / r2.v1.VOCAB_MANIFEST_SUFFIX,
        "authorization": root
        / f"artifacts/phase17/authorizations/{exp_id}_{ATTEMPT_ID}.json",
        "status_dir": root / "artifacts/phase17/status",
        "ledger": root / r2.v1.LEDGER_SUFFIX,
        "snapshot": snapshot_manifest,
        "snapshot_worker": snapshot_manifest.parent
        / "src/000_s17_fp12_resource_profile_r3_runtime.py",
        "native_python": root / r2.v1.NATIVE_PYTHON_SUFFIX,
    }
...
def frozen_config(
    ...
    with _configured_r2():
        config = _R2_FROZEN_CONFIG(root, arm_id, preflight)
    config.update(
        schema_version="phase17.s17_fp12_resource_profile_config.v3",
        revision_id=REVISION_ID,
        allocation_policy="dynamic_five_card_fastest_safe_snapshot",
        all_five_profiles_may_run_concurrently=True,
        supersedes_attempt_id="attempt_002",
    )
    return config


def snapshot_sources(root: Path, spec: r2.v1.ProfileSpec) -> list[Path]:
    original = r2._V1_SNAPSHOT_SOURCES(root, spec)
    return [
        Path(__file__).resolve(),
        Path(r2.__file__).resolve(),
        Path(r2.v1.__file__).resolve(),
        *original[1:],
    ]


def two_snapshot_admission(
    spec: r2.v1.ProfileSpec, authorization: dict[str, Any]
) -> dict[str, Any]:
    with _configured_r2():
        result = _R2_TWO_SNAPSHOT_ADMISSION(spec, authorization)
...
def verify_launch_authorization(root: Path, arm_id: str) -> dict[str, Any]:
    ...
    profile_allocation = allocation["arm_specific_resource_profiles_r3"]
    if profile_allocation["physical_gpu_by_arm"].get(arm_id) != spec.physical_gpu:
        raise PermissionError("R3 arm-to-GPU allocation drifted")
    if profile_allocation["profile_launch_authorized_by_arm"].get(arm_id) is not True:
        raise PermissionError(f"R3 launch is not authorized for {arm_id}")
    ...
    for key, value in expected.items():
        if authorization.get(key) != value:
            raise PermissionError(f"invalid R3 profile authorization field: {key}")
    if authorization.get("researcher_direction") != RESEARCHER_DIRECTION:
        raise PermissionError("R3 authorization does not preserve researcher direction")
    return {
...
def _configured_r2() -> Iterator[None]:
    replacements = {
        "ATTEMPT_ID": ATTEMPT_ID,
        "REVISION_ID": REVISION_ID,
    ...
    previous = {name: getattr(r2, name) for name in replacements}
    try:
...
def _supersede_attempt_002(root: Path, arm_id: str) -> None:
    old_experiment_id = f"s17_fp12_profile_r2_{arm_slug(arm_id)}"
    ...
    if status["scientific_state"] == "PREFLIGHT":
        writer.transition(
            "STOPPED",
            "STOPPED",
            "S17_FP12_PROFILE_SUPERSEDED_BY_DYNAMIC_FIVE_CARD_R3",
            process_alive=False,
...
def prepare(root: Path, arm_id: str) -> int:
    with _configured_r2():
        result = r2.prepare(root, arm_id)
    if result == 0:
        _supersede_attempt_002(root.resolve(), arm_id)
    return result
...
def authorize(root: Path, arm_id: str) -> int:
    ...
    if resolved["authorization"].exists():
        raise FileExistsError(f"R3 authorization exists: {resolved['authorization']}")
    writer = StatusWriter(resolved["status_dir"], experiment_id(arm_id))
    status = writer.read()
    if status["scientific_state"] != "PREFLIGHT":
        raise RuntimeError(f"R3 profile is not authorizable: {status['scientific_state']}")
    allocation = json.loads(resolved["allocation"].read_text(encoding="utf-8"))
    contract = allocation["arm_specific_resource_profiles_r3"]
    if contract["profile_launch_authorized_by_arm"].get(arm_id) is not True:
        raise PermissionError(f"allocation does not authorize R3 profile {arm_id}")
    current = r2.gpu_snapshot_once(spec)
    if current["selected"]["free_mib"] < spec.minimum_free_mib:
        raise RuntimeError(
            f"GPU{spec.physical_gpu} is below the R3 memory gate at authorization"
        )
    payload = {
        "schema_version": "phase17.s17_fp12_profile_authorization.v3",
    ...
    writer.transition(
        "PREFLIGHT",
        "PREFLIGHT",
        "S17_FP12_R3_DYNAMIC_MEMORY_ONLY_PROFILE_AUTHORIZED",
        launch_authorized=True,
        stage="dynamic_memory_only_authorized_waiting_launch",
...
def launch(root: Path, arm_id: str) -> int:
    with _configured_r2():
        return r2.launch(root, arm_id)


def worker(root: Path, arm_id: str, manifest_path: Path) -> int:
    with _configured_r2():
        return r2.worker(root, arm_id, manifest_path)


def inspect(root: Path, arm_id: str) -> dict[str, Any]:
    with _configured_r2():
        result = r2.inspect(root, arm_id)
    result["revision_id"] = REVISION_ID
    return result
