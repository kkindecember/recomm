...
RUNTIME="${PROJECT_ROOT}/experiment/phase17/protocol/s17_fp12_resource_profile_r2_runtime.py"
...
case "${ACTION}" in
  prepare-all|authorize-all|inspect-all)
  ...
  prepare|authorize|launch|inspect)
    ...
    ;;
  *)
    printf 'usage: %s [prepare-all|authorize-all|inspect-all|prepare ARM|authorize ARM|launch ARM|inspect ARM]\n' "$0" >&2
    exit 2
    ;;
esac
