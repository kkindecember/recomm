...
CONFIG="$ROOT/artifacts/phase5/configs/cet_rank_r0_preregistered.json"
OUTPUT="$ROOT/artifacts/phase5/cet_rank_r0"
LOG="$OUTPUT/run.log"
STATUS="$OUTPUT/status.json"
TELEMETRY="$OUTPUT/gpu_telemetry.csv"
SESSION=gram_phase5_cet_rank_r0
GPU=3
...
write_status() {
  ...
  printf '{"experiment_id":"GRAM_PHASE5_CET_RANK_R0","status":"%s","reason":"%s","started_at":"%s","updated_at":"%s","resource_reservation":"%s","validation_target_read":false,"test_read":false,"sports_read":false}\n' \
...
run_stage() {
  ...
  timeout --signal=TERM 7200 "$PYTHON" \
    "$ROOT/experiment/phase5/cet_rank_r0.py" "${args[@]}"
}
...
restore_resource() {
  ...
  write_status restoring_resource "Rank-R0 ended; restoring CodeLlama on GPU3."
  for _ in 1 2 3; do
...
finish() {
  ...
  if (( experiment_rc == 0 && restore_rc == 0 )); then
    write_status succeeded "CET Rank-R0 completed; inspect summary.json."
  elif (( restore_rc != 0 )); then
    write_status failed_to_restore_resource "Rank-R0 exit=$experiment_rc; restoration failed."
  else
    write_status failed "Rank-R0 exit=$experiment_rc; no automatic retry; resource restored."
  fi
  exit "$experiment_rc"
}
...
worker() {
  ...
  cd "$ROOT"
  write_status releasing_resource "Stopping CodeLlama before CET Rank-R0."
  "$RESERVER" stop
  ...
  write_status running "CET Rank-R0 beam-alignment audit running."
  telemetry_worker &
  TELEMETRY_PID=$!
...
case "${1:-status}" in
  start)
    ...
    write_status starting "Persistent CET Rank-R0 session started."
