...
CONFIG="$ROOT/artifacts/phase5/configs/cet_v2a_preregistered.json"
OUTPUT="$ROOT/artifacts/phase5/cet_v2a"
LOG="$OUTPUT/run.log"
STATUS="$OUTPUT/status.json"
TELEMETRY="$OUTPUT/gpu_telemetry.csv"
SESSION=gram_phase5_cet_v2a
GPU=3
...
write_status() {
  ...
  printf '{"experiment_id":"GRAM_PHASE5_CET_V2A","status":"%s","reason":"%s","started_at":"%s","updated_at":"%s","resource_reservation":"%s","validation_target_read":false,"test_read":false,"sports_read":false}\n' \
...
run_stage() {
  ...
  if [[ -n "$arm" ]]; then args+=(--arm "$arm"); fi
  timeout --signal=TERM 7200 "$PYTHON" \
    "$ROOT/experiment/phase5/cet_v2a.py" "${args[@]}"
}
...
restore_resource() {
  ...
  write_status restoring_resource "V2-A ended; restoring CodeLlama on GPU3."
  for _ in 1 2 3; do
...
finish() {
  ...
  if (( experiment_rc == 0 && restore_rc == 0 )); then
    write_status succeeded "CET V2-A completed; inspect summary.json."
  elif (( restore_rc != 0 )); then
    write_status failed_to_restore_resource "V2-A exit=$experiment_rc; resource restoration failed."
  else
    write_status failed "V2-A exit=$experiment_rc; no automatic retry; resource restored."
  fi
  exit "$experiment_rc"
}
...
worker() {
  ...
  cd "$ROOT"
  write_status releasing_resource "Stopping CodeLlama before CET V2-A."
  "$RESERVER" stop
  ...
  write_status running "CET V2-A calibration-B optimization smoke running."
  telemetry_worker &
  TELEMETRY_PID=$!
  for dataset in Toys Beauty; do
    for arm in V1 V2; do
      run_stage run "$dataset" "$arm"
    done
  done
  run_stage analyze
}
...
case "${1:-status}" in
  start)
    ...
    write_status starting "Persistent CET V2-A session started."
