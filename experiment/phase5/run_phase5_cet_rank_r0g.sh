...
CONFIG="$ROOT/artifacts/phase5/configs/cet_rank_r0g_preregistered.json"
OUTPUT="$ROOT/artifacts/phase5/cet_rank_r0g"
LOG="$OUTPUT/run.log"
STATUS="$OUTPUT/status.json"
TELEMETRY="$OUTPUT/gpu_telemetry.csv"
SESSION=gram_phase5_cet_rank_r0g
GPU=6
...
write_status() {
  ...
  printf '{"experiment_id":"GRAM_PHASE5_CET_RANK_R0G","status":"%s","reason":"%s","started_at":"%s","updated_at":"%s","resource_reservation":"%s","validation_target_read":false,"test_read":false,"sports_read":false}\n' \
...
run_stage() {
  ...
  timeout --signal=TERM 7200 env CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON" \
    "$ROOT/experiment/phase5/cet_rank_r0g.py" "${args[@]}"
}
...
restore_resource() {
  ...
  write_status restoring_resource "Rank-R0G ended; restoring CodeLlama on GPU6."
  for _ in 1 2 3; do
...
finish() {
  ...
  if (( experiment_rc == 0 && restore_rc == 0 )); then
    write_status succeeded "CET Rank-R0G completed; inspect summary.json."
  elif (( restore_rc != 0 )); then
    write_status failed_to_restore_resource "Rank-R0G exit=$experiment_rc; restoration failed."
  else
    write_status failed "Rank-R0G exit=$experiment_rc; no automatic retry; resource restored."
  fi
  exit "$experiment_rc"
}
...
worker() {
  ...
  cd "$ROOT"
  write_status releasing_resource "Stopping CodeLlama before CET Rank-R0G."
  "$RESERVER" stop
  ...
  write_status running "CET Rank-R0G gradient-alignment audit running."
  telemetry_worker &
  TELEMETRY_PID=$!
  for dataset in Toys Beauty; do run_stage audit "$dataset"; done
  run_stage analyze
}
...
case "${1:-status}" in
  start)
    mkdir -p "$OUTPUT"
    if tmux has-session -t "$SESSION" 2>/dev/null; then echo "session already exists: $SESSION"; exit 1; fi
    ...
    write_status starting "Persistent CET Rank-R0G session started."
