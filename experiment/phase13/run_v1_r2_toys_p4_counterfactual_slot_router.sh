...
# Phase-13 v1-R² Toys validation-only P4.
# Usage: bash experiment/phase13/run_v1_r2_toys_p4_counterfactual_slot_router.sh {start <gpu>|status|stop}

ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)
ACTION=${1:-status}
GPU=${2:-}
OUTPUT="$ROOT/artifacts/phase13/explore/v1_r2_toys_p4_counterfactual_slot_router"
STATUS="$OUTPUT/status.json"
LOG="$OUTPUT/run.log"
TELEMETRY="$OUTPUT/gpu_telemetry.csv"
SUMMARY="$OUTPUT/summary.json"
SESSION=gram_phase13_v1_r2_toys_p4_counterfactual_slot_router
...
write_status() {
  ...
  mkdir -p "$OUTPUT"
  printf '{"experiment_id":"GRAM_PHASE13_V1_R2_TOYS_P4_COUNTERFACTUAL_SLOT_ROUTER","status":"%s","stage":"%s","reason":"%s","started_at":"%s","updated_at":"%s","runner_pid":%d,"workload_pid":%d,"tmux_session":"%s","physical_gpu":%s,"minimum_free_mib":%d,"expected_incremental_gpu_mib_upper_bound":%d,"hard_timeout_seconds":%d,"resource_mode":"small_gpu_direct_no_resource_changes","split":"validation_outer_5fold_oof","test_predictions_opened":false,"automatic_retry":false,"log_path":"%s","status_path":"%s","summary_path":"%s"}\n' \
...
worker() {
  ...
  STAGE=preflight
  write_status running "Checking frozen validation inputs, P4 tests, and GPU admission."
  bash -n "$ROOT/experiment/phase13/run_v1_r2_toys_p4_counterfactual_slot_router.sh" \
    || { write_status failed "Bash syntax check failed; no automatic retry."; return 4; }
  "$PYTHON" -m py_compile \
    "$ROOT/experiment/phase13/protocol/counterfactual_slot_router.py" \
    "$ROOT/experiment/phase13/tests/test_counterfactual_slot_router.py" \
  ...
  "$PYTHON" -m pytest -q \
    ...
    || { write_status failed "P3/P4 unit tests failed; no automatic retry."; return 6; }
  for path in \
  ...
  [[ ! -e "$SUMMARY" && ! -e "$OUTPUT/config.json" && ! -e "$OUTPUT/counterfactual_slot_router.pt" ]] \
    || { write_status failed "Refusing to overwrite an existing P4 scientific artifact."; return 8; }
  ...
  STAGE=feature_export_and_outer_crossfit
  write_status running "Exporting target-free item features and running outer-fold counterfactual slot routing."
  timeout --signal=TERM --kill-after=15 "$HARD_TIMEOUT_SECONDS" \
    env CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON" \
      ...
      --gate-epochs 250 --gate-lr 0.05 --gate-l2 0.01 \
      --train-warm-retention 0.99 --seed 22345 \
      --max-history 20 --recency-decay 0.85 &
  WORKLOAD_PID=$!
  write_status running "P4 workload active on GPU${GPU}; existing processes were not modified."
  ...
  if (( rc != 0 )); then
    STAGE=finished
    write_status failed "P4 exited rc=${rc}; no automatic retry."
    return "$rc"
  fi
  [[ -s "$SUMMARY" ]] \
    || { STAGE=finished; write_status failed "P4 exited 0 without summary.json."; return 11; }
  local verdict
  verdict=$("$PYTHON" -c 'import json,sys; print(json.load(open(sys.argv[1]))["verdict"])' "$SUMMARY") \
    || { STAGE=finished; write_status failed "Could not read P4 verdict."; return 12; }
  STAGE=finished
...
case "$ACTION" in
  start)
    ...
    STARTED_AT=$(date -Is)
    STAGE=starting
    write_status starting "Background P4 session is starting on GPU${GPU}."
