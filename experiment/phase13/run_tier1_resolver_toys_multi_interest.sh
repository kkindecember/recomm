...
# Phase-13 Tier-1 Toys frozen-projector multi-interest screen.
# Usage: bash experiment/phase13/run_tier1_resolver_toys_multi_interest.sh {start <gpu>|status|stop}

ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)
ACTION=${1:-status}
GPU=${2:-}
OUTPUT="$ROOT/artifacts/phase13/explore/tier1_resolver_toys_multi_interest_screen"
STATUS="$OUTPUT/status.json"
LOG="$OUTPUT/run.log"
TELEMETRY="$OUTPUT/gpu_telemetry.csv"
SUMMARY="$OUTPUT/summary.json"
CONFIG="$ROOT/experiment/phase13/configs/tier1_resolver_toys_multi_interest.json"
PYTHON=${PYTHON_BIN:-/home/jiangtangyunzhi/miniconda3/envs/gram-repro/bin/python}
SESSION=gram_phase13_tier1_resolver_toys_multi_interest
TMUX_SOCKET=/tmp/gram_phase13_tier1_resolver_toys_multi_interest.sock
...
write_status() {
  ...
  mkdir -p "$OUTPUT"
  printf '{"experiment_id":"GRAM_PHASE13_TIER1_RESOLVER_TOYS_MULTI_INTEREST","status":"%s","stage":"%s","reason":"%s","started_at":"%s","updated_at":"%s","runner_pid":%d,"workload_pid":%d,"tmux_session":"%s","tmux_socket":"%s","physical_gpu":%s,"minimum_free_mib":%d,"expected_incremental_gpu_mib_upper_bound":%d,"hard_timeout_seconds":%d,"resource_mode":"single_gpu_background_no_resource_changes","split":"toys_validation_development_only","interest_counts":[1,2,4],"projector":"frozen_hn0_epoch12_p0","test_read":false,"beauty_read":false,"sports_read":false,"automatic_next_stage":false,"automatic_retry":false,"log_path":"%s","status_path":"%s","summary_path":"%s"}\n' \
...
worker() {
  ...
  "$PYTHON" -m py_compile \
    "$ROOT/experiment/phase13/protocol/tier1_resolver_multi_interest.py" \
    "$ROOT/experiment/phase13/tests/test_tier1_resolver_multi_interest.py" \
  ...
  "$PYTHON" -m pytest -q \
    ...
    experiment/phase13/tests/test_tier1_resolver_multi_interest.py \
    || { write_status failed "Resolver multi-interest unit tests failed; no automatic retry."; return 6; }
  for path in \
    "$CONFIG" \
    ...
    "$ROOT/artifacts/phase13/explore/tier1_resolver_toys_static_warm_hard_negative/resolver_hn_000.pt"; do
    [[ -s "$path" ]] || { write_status failed "Required input missing: ${path#$ROOT/}"; return 7; }
  done
  [[ ! -e "$SUMMARY" && ! -e "$OUTPUT/config.json" ]] \
    || { write_status failed "Refusing to overwrite an existing multi-interest artifact."; return 8; }
  ...
  write_status running "Frozen-projector M=1/2/4 multi-interest screen active."
  timeout --signal=TERM --kill-after=30 "$HARD_TIMEOUT_SECONDS" \
    env CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON" \
      ...
      --device cuda:0 &
  WORKLOAD_PID=$!
  write_status running "Multi-interest screen active on GPU${GPU}; existing processes were not modified."
  ...
  if (( rc != 0 )); then
    STAGE=finished
    write_status failed "Multi-interest screen exited rc=${rc}; no automatic retry."
    return "$rc"
  fi
  [[ -s "$SUMMARY" ]] \
  ...
  verdict=$("$PYTHON" -c 'import json,sys; print(json.load(open(sys.argv[1]))["verdict"])' "$SUMMARY") \
    || { STAGE=finished; write_status failed "Could not read multi-interest verdict."; return 15; }
  STAGE=finished
...
case "$ACTION" in
  start)
    ...
    STARTED_AT=$(date -Is)
    STAGE=starting
    write_status starting "Background multi-interest screen is starting on GPU${GPU}."
