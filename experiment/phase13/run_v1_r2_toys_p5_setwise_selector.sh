...
# Phase-13 v1-R² Toys validation-only P5-set.
# Usage: bash experiment/phase13/run_v1_r2_toys_p5_setwise_selector.sh {start <gpu>|status|stop}

ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)
ACTION=${1:-status}
GPU=${2:-}
OUTPUT="$ROOT/artifacts/phase13/explore/v1_r2_toys_p5_setwise_selector"
STATUS="$OUTPUT/status.json"
LOG="$OUTPUT/run.log"
TELEMETRY="$OUTPUT/gpu_telemetry.csv"
SUMMARY="$OUTPUT/summary.json"
SESSION=gram_phase13_v1_r2_toys_p5_setwise_selector
...
write_status() {
  ...
  mkdir -p "$OUTPUT"
  printf '{"experiment_id":"GRAM_PHASE13_V1_R2_TOYS_P5_SETWISE_SELECTOR","status":"%s","stage":"%s","reason":"%s","started_at":"%s","updated_at":"%s","runner_pid":%d,"workload_pid":%d,"tmux_session":"%s","physical_gpu":%s,"minimum_free_mib":%d,"expected_incremental_gpu_mib_upper_bound":%d,"hard_timeout_seconds":%d,"resource_mode":"single_gpu_background_no_resource_changes","split":"warm_only_pseudo_cold_plus_validation_held_fold","test_predictions_opened":false,"automatic_retry":false,"log_path":"%s","status_path":"%s","summary_path":"%s"}\n' \
...
worker() {
  ...
  STAGE=preflight
  write_status running "Checking frozen validation inputs, P5-set tests, and GPU admission."
  bash -n "$ROOT/experiment/phase13/run_v1_r2_toys_p5_setwise_selector.sh" \
    || { write_status failed "Bash syntax check failed; no automatic retry."; return 4; }
  "$PYTHON" -m py_compile \
    "$ROOT/experiment/phase13/protocol/setwise_candidate_selector.py" \
    "$ROOT/experiment/phase13/tests/test_setwise_candidate_selector.py" \
  ...
  "$PYTHON" -m pytest -q \
    ...
    experiment/phase13/tests/test_setwise_candidate_selector.py \
    || { write_status failed "P4/P5-set unit tests failed; no automatic retry."; return 6; }
  for path in \
  ...
  [[ ! -e "$SUMMARY" && ! -e "$OUTPUT/config.json" && ! -e "$OUTPUT/setwise_selector.pt" ]] \
    || { write_status failed "Refusing to overwrite an existing P5-set scientific artifact."; return 8; }
  ...
  STAGE=warm_only_setwise_training_and_validation_audit
  write_status running "Exporting warm-only pseudo-cold sets, training selector, then running validation held-fold audit."
  timeout --signal=TERM --kill-after=20 "$HARD_TIMEOUT_SECONDS" \
    env CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON" \
      ...
      --warm-items "$ROOT/GRAM/rec_datasets/Toys_cold50/cold_split_meta/warm_items.txt" \
      --output-dir "$OUTPUT" --device cuda:0 --pool-size 10 --hidden-dim 32 \
      --epochs 15 --batch-size 256 --lr 0.001 --weight-decay 0.0001 \
      --max-history 20 --recency-decay 0.85 --seed 32345 &
  WORKLOAD_PID=$!
  write_status running "P5-set workload active on GPU${GPU}; existing processes were not modified."
  ...
  if (( rc != 0 )); then
    STAGE=finished
    write_status failed "P5-set exited rc=${rc}; no automatic retry."
    return "$rc"
  fi
  [[ -s "$SUMMARY" ]] \
    || { STAGE=finished; write_status failed "P5-set exited 0 without summary.json."; return 11; }
  local verdict
  verdict=$("$PYTHON" -c 'import json,sys; print(json.load(open(sys.argv[1]))["verdict"])' "$SUMMARY") \
    || { STAGE=finished; write_status failed "Could not read P5-set verdict."; return 12; }
  STAGE=finished
...
case "$ACTION" in
  start)
    ...
    STARTED_AT=$(date -Is)
    STAGE=starting
    write_status starting "Background P5-set session is starting on GPU${GPU}."
