#!/usr/bin/env bash
# Smoke collision-safe E5 v1 iter2 IDs on a user-assigned GPU.
# This script never changes protectors, holders, or lease resources.
#
# Usage:
#   bash experiment/phase13/run_v1_e5_toys_smoke.sh start 6
#   bash experiment/phase13/run_v1_e5_toys_smoke.sh status
#   bash experiment/phase13/run_v1_e5_toys_smoke.sh stop
set -uo pipefail
...
SUB=smoke_v1_collision_safe_e5_toys
...
ID_FILE="$ROOT/GRAM/rec_datasets/Toys_cold50/item_generative_indexing_hierarchy_v1_c32_l5_len32768_split_v1_e5_mlpcold_collision_safe.txt"
ID_REPORT="$ROOT/artifacts/phase13/explore/v1_collision_safe_e5_toys_prep/id_report.json"
...
worker() {
  ...
  write_status running preflight "Checking code, E5 collision-safe IDs, and assigned GPU."
  "$PYTHON" -m py_compile \
  ...
  [[ -s "$ID_FILE" && -s "$ID_REPORT" ]] \
    || { write_status failed preflight "E5 collision-safe artifacts missing; no automatic retry."; return 6; }
  "$PYTHON" -c "import json; r=json.load(open('$ID_REPORT')); assert r['output_collision']['duplicate_excess']==0; assert r['warm_ids_unchanged']; assert r['row_order_unchanged']; assert r['cold_prefixes_unchanged']" \
    || { write_status failed preflight "E5 collision-safe report failed invariants; no automatic retry."; return 7; }
  ...
  (( free_mib >= MIN_FREE_MIB )) \
    || { write_status blocked admission "GPU${GPU} has ${free_mib} MiB free; requires ${MIN_FREE_MIB}; no resource changes made."; return 9; }
  ...
  timeout --signal=TERM "$TIMEOUT_SECONDS" env \
    ...
    --hierarchical_id_type hierarchy_v1_c32_l5_len32768_split_v1_e5_mlpcold_collision_safe \
  ...
  if (( workload_rc != 0 )); then
    ...
    if (( workload_rc == 124 || workload_rc == 143 )); then
      ...
      write_status failed training "GRAM E5 smoke exited rc=${workload_rc}; no automatic retry."
    fi
    return "$workload_rc"
  fi

  write_status running postflight "Computing cold/warm metrics from E5 smoke predictions."
  local pred_tsv
  ...
  cleanup
  write_status completed finished "E5 smoke completed; formal 30-epoch experiment was not auto-started."
}
...
case "$ACTION" in
  start)
    ...
    write_status starting starting "Background E5 smoke session started on user-assigned GPU${GPU}."
