...
# Phase-13 v1-R² Toys validation-only P3.
# Usage: bash experiment/phase13/run_v1_r2_toys_p3_confidence_abstention.sh {start <gpu>|status|stop}

ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)
ACTION=${1:-status}
GPU=${2:-}
OUTPUT="$ROOT/artifacts/phase13/explore/v1_r2_toys_p3_confidence_abstention"
STATUS="$OUTPUT/status.json"
LOG="$OUTPUT/run.log"
TELEMETRY="$OUTPUT/gpu_telemetry.csv"
SUMMARY="$OUTPUT/summary.json"
SESSION=gram_phase13_v1_r2_toys_p3_confidence_abstention
...
write_status() {
  ...
  mkdir -p "$OUTPUT"
  printf '{"experiment_id":"GRAM_PHASE13_V1_R2_TOYS_P3_CONFIDENCE_ABSTENTION","status":"%s","stage":"%s","reason":"%s","started_at":"%s","updated_at":"%s","runner_pid":%d,"workload_pid":%d,"tmux_session":"%s","physical_gpu":%s,"minimum_free_mib":%d,"expected_incremental_gpu_mib_upper_bound":%d,"hard_timeout_seconds":%d,"resource_mode":"small_gpu_direct_no_resource_changes","split":"validation_5fold_oof","test_predictions_opened":false,"log_path":"%s","status_path":"%s","summary_path":"%s"}\n' \
...
worker() {
  ...
  STAGE=preflight
  write_status running "Checking frozen inputs, P3 tests, and GPU admission."
  bash -n "$ROOT/experiment/phase13/run_v1_r2_toys_p3_confidence_abstention.sh" \
    || { write_status failed "Bash syntax check failed; no automatic retry."; return 4; }
  "$PYTHON" -m py_compile \
    "$ROOT/experiment/phase13/protocol/confidence_abstention.py" \
    "$ROOT/experiment/phase13/tests/test_confidence_abstention.py" \
    || { write_status failed "Python syntax check failed; no automatic retry."; return 5; }
  cd "$ROOT"
  "$PYTHON" -m pytest -q experiment/phase13/tests/test_confidence_abstention.py \
    || { write_status failed "P3 unit tests failed; no automatic retry."; return 6; }
  for path in \
  ...
  [[ ! -e "$SUMMARY" && ! -e "$OUTPUT/config.json" && ! -e "$OUTPUT/confidence_gates.pt" ]] \
    || { write_status failed "Refusing to overwrite an existing P3 scientific artifact."; return 8; }
  ...
  STAGE=feature_export_and_crossfit
  write_status running "Exporting frozen resolver confidence and running five-fold OOF abstention."
  timeout --signal=TERM --kill-after=15 "$HARD_TIMEOUT_SECONDS" \
    env CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON" \
      ...
      --gate-epochs 300 --gate-lr 0.05 --gate-l2 0.01 \
      --train-warm-retention 0.985 --seed 12345 \
      --max-history 20 --recency-decay 0.85 &
  WORKLOAD_PID=$!
  write_status running "P3 workload active on GPU${GPU}; existing processes were not modified."
  ...
  if (( rc != 0 )); then
    STAGE=finished
    write_status failed "P3 exited rc=${rc}; no automatic retry."
    return "$rc"
  fi
  [[ -s "$SUMMARY" ]] \
    || { STAGE=finished; write_status failed "P3 exited 0 without summary.json."; return 11; }
  local verdict
  verdict=$("$PYTHON" -c 'import json,sys; print(json.load(open(sys.argv[1]))["verdict"])' "$SUMMARY") \
    || { STAGE=finished; write_status failed "Could not read P3 verdict."; return 12; }
  STAGE=finished
...
case "$ACTION" in
  start)
    ...
    STARTED_AT=$(date -Is)
    STAGE=starting
    write_status starting "Background P3 session is starting on GPU${GPU}."
