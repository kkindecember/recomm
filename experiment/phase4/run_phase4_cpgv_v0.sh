...
GPU=3
MIN_FREE_MIB=30720
SESSION=gram_phase4_cpgv_v0
RESERVER="$ROOT/tools/run_codellama.sh"
PYTHON=/home/jiangtangyunzhi/miniconda3/envs/gram-repro/bin/python3.9
STATUS="$ROOT/experiment/phase4/phase4_cpgv_v0_status.json"
PID_FILE="$ROOT/experiment/phase4/phase4_cpgv_v0.pid"
LOG="$ROOT/artifacts/phase4/logs/cpgv_v0.log"
OUTPUT="$ROOT/artifacts/phase4/cpgv_v0"
started_at=$(date -Is)
workload_pid=""
...
write_status() {
    ...
    printf '{\n  "experiment": "GRAM phase4 CPGV V0",\n  "updated_at": "%s",\n  "started_at": "%s",\n  "status": "%s",\n  "reason": "%s",\n  "gpu_selected": 3,\n  "runner_pid": %s,\n  "workload_pid": %s,\n  "log": "artifacts/phase4/logs/cpgv_v0.log",\n  "output": "artifacts/phase4/cpgv_v0/summary.json",\n  "resource_reservation": "%s"\n}\n' \
...
restore_resource() {
    ...
    write_status restoring_resource "CPGV V0 ended; restoring CodeLlama on physical GPU3."
...
finish() {
    ...
    if (( experiment_rc == 0 && restore_rc == 0 )); then
        write_status succeeded "CPGV V0 completed; inspect summary.json."
    elif (( restore_rc != 0 )); then
        write_status failed_to_restore_resource "V0 exit=${experiment_rc}; resource restore failed."
    else
        write_status failed "V0 exit=${experiment_rc}; no scientific decision inferred."
    fi
    rm -f "$PID_FILE"
...
worker() {
    ...
    write_status running "Frozen GRAM exact proposal verification is running."
    CUDA_VISIBLE_DEVICES="$GPU" \
        HF_HOME="$ROOT/.cache/huggingface" \
        TRANSFORMERS_CACHE="$ROOT/.cache/huggingface" \
        "$PYTHON" "$ROOT/experiment/phase4/cpgv_v0.py" \
        --config "$ROOT/artifacts/phase4/configs/cpgv_v0_preregistered.json" \
        --output-dir "$OUTPUT" \
        --device cuda &
    workload_pid=$!
    write_status running "Frozen GRAM exact proposal verification is running."
...
case "$ACTION" in
    start)
        ...
        tmux new-session -d -s "$SESSION" \
            "bash '$ROOT/experiment/phase4/run_phase4_cpgv_v0.sh' worker '$started_at'"
