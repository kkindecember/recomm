...
SESSION=gram_phase7_gcgd_p1
GPU=0
PYTHON=/home/jiangtangyunzhi/miniconda3/envs/gram-repro/bin/python3.9
WORKLOAD="$ROOT/experiment/phase7/gcgd_p1_run.py"
TEST="$ROOT/experiment/phase7/test_gcgd_p1_run.py"
...
write_status() {
  ...
  mkdir -p "$OUTPUT"
  printf '{"experiment_id":"GRAM_PHASE7_GCGD_P1_V1","status":"%s","stage":"%s","dataset":"%s","reason":"%s","started_at":"%s","updated_at":"%s","runner_pid":%s,"workload_pid":%s,"tmux_session":"%s","physical_gpu":0,"total_gpu_lease_mib":30720,"log_path":"%s","result_path":"%s","resource_reservation":"%s","test_read":false,"sports_read":false}\n' \
    "$state" "$STAGE" "$CURRENT_DATASET" "$reason" "$STARTED_AT" "$(date -Is)" "$$" \
    "$WORKLOAD_PID" "$SESSION" "${LOG#$ROOT/}" "${OUTPUT#$ROOT/}/summary.json" "$RESERVATION" > "$tmp"
  mv "$tmp" "$STATUS"
}

config_is_enabled() {
  "$PYTHON" -c 'import json,sys; c=json.load(open(sys.argv[1])); ok=c.get("execution_enabled") is True and c.get("scientific_workload_implemented") is True and c.get("decision_status") == "PREREGISTERED_FROZEN_READY_TO_RUN"; raise SystemExit(0 if ok else 1)' "$CONFIG"
}
...
inputs=c["inputs"]
for key in ("p0_lineage_summary","p0_gpu_smoke_summary"):
 p=root/inputs[key]
 if hashlib.sha256(p.read_bytes()).hexdigest()!=inputs[key+"_sha256"]: raise SystemExit(1)
for dataset in c["datasets"]:
 checkpoint=root/inputs["checkpoint_root"]/dataset/"C1/model.pt"
 residual=root/inputs["gacr_v3_residual_root"]/dataset/"residual_seed2023.pt"
 if hashlib.sha256(checkpoint.read_bytes()).hexdigest()!=inputs["expected_parent_checkpoint_sha256"][dataset]: raise SystemExit(1)
 if hashlib.sha256(residual.read_bytes()).hexdigest()!=inputs["expected_gacr_v3_seed2023_residual_sha256"][dataset]: raise SystemExit(1)' "$ROOT" "$CONFIG"
}
...
restore() {
  ...
  write_status restoring_resource "Formal P1 ended; restoring CodeLlama on GPU0."
...
finish() {
  ...
  if (( scientific_rc == 0 && restore_rc == 0 )); then
    write_status succeeded "Formal P1 completed for both domains; results await researcher analysis; CodeLlama restored."
...
start_domain_lease() {
  ...
  "$PYTHON" "$LEASE_HELPER" --gpu "$GPU" --total-lease-mib "$TOTAL_LEASE_MIB" \
    --expected-workload-peak-mib "$expected_peak" --status-path "$OUTPUT/gpu_lease_${dataset}.json" &
  LEASE_PID=$!
...
worker() {
  ...
  config_is_enabled || { write_status blocked "Formal P1 config is not frozen and enabled."; exit 3; }
  locks_match || { write_status blocked "Formal P1 code/input SHA lock mismatch."; exit 7; }
  env HF_HOME="$WORKLOAD_HF_HOME" HF_HUB_CACHE="$WORKLOAD_HF_HOME" TRANSFORMERS_CACHE="$WORKLOAD_HF_HOME" \
    "$PYTHON" -c 'from transformers import AutoTokenizer; AutoTokenizer.from_pretrained("t5-small", local_files_only=True)' \
  ...
  write_status releasing_resource "Stopping CodeLlama before formal P1."
  reserver stop
  ...
  for dataset in Toys Beauty; do
    ...
    STAGE=domain_lease
    if [[ "$dataset" == Toys ]]; then start_domain_lease "$dataset" 5120; else start_domain_lease "$dataset" 2048; fi
    STAGE=scientific_workload
    timeout --signal=TERM 43200 env CUDA_VISIBLE_DEVICES="$GPU" HF_HOME="$WORKLOAD_HF_HOME" \
    ...
    WORKLOAD_PID=$!
    write_status running "Formal P1 running with a domain-specific 30 GiB lease."
    wait "$WORKLOAD_PID"
    WORKLOAD_PID=0
    release_lease
  done
...
root=pathlib.Path(sys.argv[1]); datasets={d:json.load(open(root/d/"summary.json")) for d in ("Toys","Beauty")}
summary={"experiment_id":"GRAM_PHASE7_GCGD_P1_V1","status":"PASS","datasets":datasets,"test_read":False,"sports_read":False,"effect_decision_automatic":False}
...
case "${1:-status}" in
  start)
    mkdir -p "$OUTPUT"
    config_is_enabled || { STAGE=config_not_enabled; write_status blocked "Start refused: formal P1 is awaiting researcher confirmation; CodeLlama unchanged."; exit 3; }
    ...
    STAGE=starting
    write_status starting "Persistent formal P1 session started on GPU0."
