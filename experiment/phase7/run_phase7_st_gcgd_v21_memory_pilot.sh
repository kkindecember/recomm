...
CONFIG="$ROOT/artifacts/phase7/configs/st_gcgd_v21_memory_preregistered.json"
OUTPUT="$ROOT/artifacts/phase7/st_gcgd_v21_p0r2"
LOG="$OUTPUT/run.log"
STATUS="$OUTPUT/status.json"
CURRENT_FILE="$OUTPUT/current_dataset.txt"
SESSION=gram_phase7_st_gcgd_v21_p0r2
PYTHON=/home/jiangtangyunzhi/miniconda3/envs/gram-repro/bin/python3.9
WORKLOAD="$ROOT/experiment/phase7/st_gcgd_v21_memory_pilot.py"
IMPLEMENTATION="$ROOT/experiment/phase7/st_gcgd_v21.py"
...
write_status() {
  ...
  mkdir -p "$OUTPUT"
  printf '{"experiment_id":"GRAM_PHASE7_ST_GCGD_V21_P0_R2_V1","status":"%s","stage":"%s","dataset":"%s","reason":"%s","started_at":"%s","updated_at":"%s","runner_pid":%s,"workload_pid":%s,"tmux_session":"%s","physical_gpu":0,"sidecar_active":false,"test_read":false,"sports_read":false}\n' \
...
for key in ("implementation","memory_pilot","test","runner"):
 p=root/lock[key]["path"]
 if hashlib.sha256(p.read_bytes()).hexdigest()!=lock[key]["sha256"]: raise SystemExit(1)
p=root/c["parent_lineage"]["v2_integrity_audit"]
raise SystemExit(0 if hashlib.sha256(p.read_bytes()).hexdigest()==c["parent_lineage"]["v2_integrity_audit_sha256"] else 1)' "$CONFIG" "$ROOT"
}
...
finish() {
  ...
  if (( scientific_rc == 0 && restore_rc == 0 )); then
    write_status succeeded "P0-R2 bank harvest and graph-only memory pilot completed; CodeLlama restored."
  elif (( scientific_rc == 0 )); then
    write_status failed_to_restore_resource "P0-R2 completed but CodeLlama restoration failed."
  else
    write_status failed "P0-R2 exit=$scientific_rc; no automatic retry; CodeLlama restoration requested."
  fi
...
worker() {
  ...
  mkdir -p "$OUTPUT"
  STAGE=preflight
  config_ready || { write_status blocked "P0-R2 config not ready."; exit 3; }
  locks_match || { write_status blocked "P0-R2 code/lineage SHA mismatch."; exit 4; }
  "$PYTHON" -m pytest -q "$TEST" "$ROOT/experiment/phase7/test_st_gcgd_v2.py" || { write_status blocked "P0-R2 tests failed."; exit 5; }
  ...
  for dataset in Toys Beauty; do
    ...
    STAGE=hard_negative_and_graph_only_memory
    timeout --signal=TERM 43200 env CUDA_VISIBLE_DEVICES=0 HF_HOME="$ROOT/.cache/huggingface" \
    ...
    WORKLOAD_PID=$!
    write_status running "Expanded GRAM bank and deep graph-only CUDA pilot running."
    wait "$WORKLOAD_PID"
    WORKLOAD_PID=0
  done
}
...
case "${1:-status}" in
  start)
    mkdir -p "$OUTPUT"
    config_ready || { STAGE=blocked; write_status blocked "P0-R2 config not ready."; exit 3; }
    locks_match || { STAGE=blocked; write_status blocked "P0-R2 code/lineage SHA mismatch."; exit 4; }
    ...
    STAGE=starting
    write_status starting "Persistent P0-R2 session started."
