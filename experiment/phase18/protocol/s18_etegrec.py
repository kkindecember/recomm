...
def snapshot(config, output):
    ...
    paths = ['experiment/phase18/core/etegrec_adapter.py',
             'experiment/phase18/protocol/s18_etegrec.py',
             'experiment/phase18/config/s18_etegrec_beauty.json']
    hashes = {}
    for path in paths:
...
def profile(config, config_path):
    ...
    try:
        ...
        result = {'state': 'PASSED', 'hardware': hardware, 'timings': timings, 'code_info': code_info,
                  ...
                  'local_source_sha256': {p: a.digest(a.ROOT / p) for p in ('experiment/phase18/core/etegrec_adapter.py', 'experiment/phase18/protocol/s18_etegrec.py')},
