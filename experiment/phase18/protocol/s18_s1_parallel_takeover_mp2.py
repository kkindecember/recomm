#!/usr/bin/env python3
"""Immediate S18-1 parallel takeover using GPU0+6 decoder model parallelism."""
...
EXPERIMENT_ID = "s18_s1_actionability_parallel_takeover_mp2"
ATTEMPT_ID = "run-0005"
AUTH_PATH = ROOT / "experiment/phase18/config/s18_s1_parallel_takeover_mp2_authorization.json"
OUTPUT = ROOT / "artifacts/phase18/s1_actionability/run-0005"
STATUS = ROOT / "artifacts/phase18/status/s18_s1_actionability_parallel_takeover_mp2.status.json"
...
RUN4_STATUS = ROOT / "artifacts/phase18/status/s18_s1_actionability_parallel_takeover.status.json"
...
REPORT = ROOT / "report/第十八阶段/Stage18_S1_GPU0-GPU6并行资源接管报告.md"
...
def verify_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if authorization["experiment_id"] != EXPERIMENT_ID or authorization["attempt_id"] != ATTEMPT_ID:
        raise RuntimeError("run-0005 authorization identity mismatch")
    ...
    if any(not scope[name] for name in required_true) or any(scope[name] for name in required_false):
        raise RuntimeError("run-0005 scope is not result-preserving")
    runtime = authorization["runtime"]
    if runtime["beauty_physical_gpus"] != [0, 6]:
        raise RuntimeError("run-0005 must use physical GPU0+6 for Beauty")
    if runtime["beauty_units"] != ["Beauty:I0", "Beauty:I-1"] or not runtime["beauty_units_serial"]:
        raise RuntimeError("run-0005 Beauty queue mismatch")
    for record in authorization["frozen_inputs"].values():
        path = ROOT / record["path"]
        if not path.is_file() or sha256(path) != record["sha256"]:
            raise RuntimeError(f"frozen run-0005 input mismatch: {path}")
    smoke = load_json(ROOT / authorization["frozen_inputs"]["two_gpu_parity_smoke"]["path"])
    ...
    if (
        ...
        raise RuntimeError("two-GPU exact-parity smoke is invalid")
    expected_required = {
        str(row["physical_gpu"]): int(round(row["reserved_mib"])) + 4096
        for row in smoke["peak_by_visible_gpu"].values()
    }
    physical_required = {
        "0": expected_required["1"],
        "6": expected_required["7"],
    }
    if runtime["required_free_mib_by_gpu"] != physical_required:
        raise RuntimeError("run-0005 memory thresholds do not match the exact-parity smoke")
    source = authorization["source_takeover"]
    if pid_alive(int(source["superseded_controller_pid"])):
        raise RuntimeError("run-0004 waiting controller is still alive")
    ...
    if not source_summary.is_file() and not matching_live_heartbeat:
        raise RuntimeError("preserved Toys:I-1 worker is neither live nor complete")
    occupancy = authorization["postrun_occupancy"]
    if (
        occupancy["physical_gpu"] != 0
        or not occupancy["launch_after_beauty_lane"]
        or not occupancy["normal_priority_preemption"]
        or occupancy["result_selection_eligible"]
        or not occupancy["repeat_metrics_ignored"]
    ):
        raise RuntimeError("early GPU0 occupancy contract mismatch")
...
def validate_completed_unit(source_dir: Path, domain: str, fold: str) -> Path:
    ...
    if (
        ...
        or any(summary.get(key) for key in ("d1_read", "d2_read", "test_read", "sports_read", "treatment_training"))
    ):
...
def carry_forward(label: str, source_path: Path, source_attempt: str) -> None:
    ...
    if target.exists():
        raise FileExistsError(f"run-0005 carry target exists; retry forbidden: {target}")
    ...
    base.atomic_json(
        target / "status.json",
        {
            "schema_version": "phase18.s18_1_parallel_takeover_unit_status.v2",
...
def wait_for_beauty_pair(authorization: dict[str, Any], label: str) -> list[dict[str, Any]]:
    runtime = authorization["runtime"]
    pair = runtime["beauty_physical_gpus"]
    required = {int(key): int(value) for key, value in runtime["required_free_mib_by_gpu"].items()}
    stable_count = 0
    started = time.time()
    ...
    while True:
        ...
        eligible = all(gpu in by_index and by_index[gpu]["free_mib"] >= required[gpu] for gpu in pair)
        stable_count = stable_count + 1 if eligible else 0
        update_status(
            stage="parallel_lanes",
            execution_state="WAITING_FOR_GPU0_GPU6" if stable_count < runtime["stable_snapshots_required"] else "GPU0_GPU6_ADMITTED",
            scientific_state="RUNNING",
            process_alive=True,
            beauty_lane_state="WAITING_FOR_ADMISSION",
...
def run_beauty_unit(fold: str, physical_gpus: list[int]) -> int:
    ...
    try:
        ...
        if torch.cuda.device_count() != 2:
            raise RuntimeError("run-0005 Beauty unit requires exactly two visible GPUs")
...
def execute_beauty_unit(label: str, authorization: dict[str, Any]) -> tuple[int, int, Path]:
    ...
    with log.open("w", encoding="utf-8") as handle:
        process = subprocess.Popen(
            [str(PYTHON), str(Path(__file__).resolve()), "unit", "--fold", fold],
            cwd=ROOT,
            env=environment,
            stdout=handle,
        ...
        while process.poll() is None:
            ...
            update_status(
                ...
                scientific_state="RUNNING",
                process_alive=True,
                beauty_lane_state="RUNNING",
                beauty_lane_process_alive=True,
                lanes={
                    "gpu1_gpu7": {"label": "Toys:I-1", "source_status": source_toys_status(authorization)},
                    "gpu0_gpu6": {
                        "label": label,
...
def wait_for_toys_im1(authorization: dict[str, Any]) -> Path:
    ...
    while True:
        ...
        if not pid_alive(int(source["active_worker_pid"])) and not summary_path.is_file():
            raise RuntimeError("preserved Toys:I-1 worker exited without summary")
        update_status(
            ...
            lanes={
                ...
                "gpu0": {"state": "POSTRUN_OCCUPANCY_ELIGIBLE"},
            },
        )
...
def launch_early_occupancy(authorization: dict[str, Any]) -> dict[str, Any]:
    environment = os.environ.copy()
    environment["S18_OCCUPANCY_PROFILE"] = "run-0005"
    ...
    return {
        "launch_return_code": completed.returncode,
        "stdout": completed.stdout.strip(),
        "stderr": completed.stderr.strip(),
        "physical_gpu": 0,
        "launch_gate": "Beauty:I0 and Beauty:I-1 completed; GPU0+6 formal worker exited",
        "does_not_wait_for_toys_or_aggregate": True,
...
def aggregate(config: dict[str, Any]) -> dict[str, Any]:
    ...
    summary.update(
        experiment_id=EXPERIMENT_ID,
        attempt_id=ATTEMPT_ID,
        recovery_of="run-0002 Toys:I0 + run-0003 Toys:I-1 + run-0005 Beauty units",
        ...
        resource_adaptation={
            "parallel_lanes": {
                ...
                "gpu0_gpu6": "run-0005 Beauty:I0 then Beauty:I-1 decoder model parallel",
            },
    ...
    base.atomic_json(
        CANONICAL_MANIFEST,
        {
            ...
            "reason": "explicit result-preserving immediate GPU0+6 parallel takeover",
            "created_at": base.utc_now(),
        },
    )
    base.REPORT = REPORT
    ...
    with REPORT.open("a", encoding="utf-8") as handle:
        handle.write(
            "\n## Parallel Takeover\n\n"
            "- Toys:I0 was carried from run-0002 and Toys:I-1 from the preserved run-0003 worker.\n"
            "- Beauty:I0 and Beauty:I-1 used the exact-parity decoder split on physical GPU0+6.\n"
            "- GPU0 repeat occupancy became eligible immediately after the Beauty lane completed.\n"
            "- No checkpoint, cohort, beam, score, Gate, or protected-data access changed.\n"
        )
    return summary


def mark_run4_superseded() -> None:
    current = load_json(RUN4_STATUS) if RUN4_STATUS.is_file() else {}
    current.update(
        execution_state="SUPERSEDED_BEFORE_BEAUTY_EXECUTION",
        scientific_state="SUPERSEDED_NO_NEW_SCIENTIFIC_UNIT",
        status_code="S18_1_RUN_0004_SUPERSEDED_BY_RUN_0005",
        process_alive=False,
        workload_pid=0,
        superseded_by=ATTEMPT_ID,
        automatic_retry=False,
        automatic_s18_2=False,
        next_action="Observe run-0005 GPU0+6 parallel takeover.",
        updated_at=base.utc_now(),
        heartbeat_at=base.utc_now(),
    )
    base.atomic_json(RUN4_STATUS, current)
...
def fail_unit(label: str, pid: int, log: Path, return_code: int) -> int:
    base.append_jsonl(
        LEDGER,
        {
            "event": "parallel_takeover_mp2_unit_failed_no_retry",
            "at": base.utc_now(),
            "attempt_id": ATTEMPT_ID,
            "unit": label,
            "physical_gpus": [0, 6],
    ...
    update_status(
        ...
        status_code="S18_1_PARALLEL_TAKEOVER_MP2_BEAUTY_FAILURE_NO_RETRY",
        process_alive=False,
        workload_pid=0,
        beauty_lane_state="FAILED_NO_RETRY",
        beauty_lane_process_alive=False,
        failed_unit=label,
        failed_unit_pid=pid,
...
def master() -> int:
    ...
    if queue_manifest["source_manifest"] != source_manifest(checkpoint_auth):
        raise RuntimeError("run-0005 source manifest changed after launch")
    try:
        carry_forward(
            "Toys:I0",
            validate_completed_unit(RUN2_TOYS_I0, "Toys", "I0"),
            "s18_s1_actionability_recovery/run-0002",
        )
        completed_units = 1
        run_manifest = {
            "schema_version": "phase18.s18_1_parallel_takeover_run_manifest.v2",
            "experiment_id": EXPERIMENT_ID,
            "attempt_id": ATTEMPT_ID,
            "created_at": base.utc_now(),
            ...
            "parallel_lanes": {
                "gpu1_gpu7": "preserved run-0003 Toys:I-1",
                "gpu0_gpu6": authorization["runtime"]["beauty_units"],
            },
            "unit_assignments": [],
            "automatic_retry": False,
            "automatic_s18_2": False,
        ...
        for label in authorization["runtime"]["beauty_units"]:
            admission_history = wait_for_beauty_pair(authorization, label)
            run_manifest["unit_assignments"].append(
                {"label": label, "physical_gpus": [0, 6], "admission_history": admission_history}
            )
            base.atomic_json(OUTPUT / "run_manifest.json", run_manifest)
            ...
            completed_units += 1
            update_status(progress={"current": completed_units, "total": 4, "unit": "domain_fold_diagnostic"})
        update_status(
            stage="beauty_lane_complete",
            execution_state="RUNNING_PARALLEL_SCIENCE",
            scientific_state="RUNNING",
            process_alive=True,
            beauty_lane_state="COMPLETED",
            beauty_lane_process_alive=False,
            gpu0_released_from_formal_science=True,
        )
        occupancy = launch_early_occupancy(authorization)
        update_status(postrun_occupancy=occupancy)
        ...
        base.append_jsonl(
            LEDGER,
            {
                "event": "authorized_parallel_takeover_mp2_completed",
                "at": base.utc_now(),
                "attempt_id": ATTEMPT_ID,
        ...
        update_status(
            ...
            process_alive=False,
            workload_pid=0,
            progress={"current": completed_units, "total": 4, "unit": "domain_fold_diagnostic"},
        ...
        base.append_jsonl(
            LEDGER,
            {
                "event": "authorized_parallel_takeover_mp2_failed_no_retry",
                "at": base.utc_now(),
                "attempt_id": ATTEMPT_ID,
                "error_type": type(error).__name__,
                "error": str(error),
        ...
        update_status(
            ...
            execution_state="FAILED_NO_RETRY",
            scientific_state="FAILED",
            status_code="S18_1_PARALLEL_TAKEOVER_MP2_FAILURE_NO_RETRY",
            process_alive=False,
            workload_pid=0,
            beauty_lane_process_alive=False,
            error_type=type(error).__name__,
...
def launch() -> int:
    ...
    if OUTPUT.exists() or STATUS.exists():
        raise FileExistsError("run-0005 artifacts already exist; automatic retry forbidden")
    OUTPUT.mkdir(parents=True)
    queue_manifest = {
        "schema_version": "phase18.s18_1_parallel_takeover_queue_manifest.v2",
        "experiment_id": EXPERIMENT_ID,
        "attempt_id": ATTEMPT_ID,
        "created_at": base.utc_now(),
    ...
    update_status(
        ...
        execution_state="RUNNING_PARALLEL_SCIENCE",
        scientific_state="RUNNING",
        status_code="S18_1_PARALLEL_TAKEOVER_MP2_STARTING",
        process_alive=True,
        workload_pid=0,
        tmux_session=authorization["runtime"]["tmux_session"],
        beauty_lane_state="STARTING",
        beauty_lane_process_alive=False,
        result_selection_eligible=False,
        ...
        sports_read=False,
        started_at=base.utc_now(),
        next_action="Observe GPU1+7 Toys and GPU0+6 Beauty lanes; do not launch another S18-1 attempt.",
    )
    mark_run4_superseded()
    base.append_jsonl(
        LEDGER,
        {
            "event": "authorized_parallel_takeover_mp2_queued",
            "at": base.utc_now(),
            "experiment_id": EXPERIMENT_ID,
            "attempt_id": ATTEMPT_ID,
            "preserved_worker_pid": authorization["source_takeover"]["active_worker_pid"],
    ...
    while time.time() < deadline:
        ...
        if status.get("workload_pid", 0) > 0:
            ...
            return 0
        time.sleep(1)
    raise RuntimeError("run-0005 background master failed startup handshake")
...
def main() -> int:
    ...
    if args.action == "unit":
        ...
        return run_beauty_unit(args.fold, [0, 6])
