...
ACTION=${1:-status}
GPU=${2:-}
OUTPUT=${OUTPUT_OVERRIDE:-"$ROOT/artifacts/phase15/s4_beauty/formal/b0_b1_b2_seed0"}
...
SESSION=${SESSION_OVERRIDE:-gram_stage15_s4_beauty_b2_full_validation}
MIN_FREE_MIB=16384
...
write_status() {
  ...
  mkdir -p "$OUTPUT"
  printf '{"experiment_id":"GRAM_STAGE15_S4_BEAUTY_FULL_VALIDATION_B0_B1_B2_SEED0","status":"%s","status_code":"%s","stage":"%s","reason":"%s","started_at":"%s","updated_at":"%s","runner_pid":%d,"workload_pid":%d,"workload_rc":%d,"exit_code":%d,"exit_code_pending":%s,"physical_gpu":%s,"minimum_free_mib":%d,"expected_incremental_gpu_mib_upper_bound":%d,"hard_timeout_seconds":%d,"events":10655,"train_transitions":4096,"candidate_budget":50,"beam_size":50,"arms":"b0,b1,b2","domain":"Beauty_cold50","split":"validation","original_user_sequence_opened":false,"test_opened":false,"test_read":false,"automatic_retry":false,"resource_mode":"user_assigned_remaining_memory_only_no_process_changes","exact_start_command":"%s","output_dir":"%s","log_path":"%s","summary_path":"%s"}\n' \
...
worker() {
  ...
  STAGE=preflight
  write_status running "Checking frozen S15-4 Beauty B2 contract, tests, hashes, test seal, and GPU admission."
  bash -n experiment/phase15/run_stage15_s4_beauty_b2_full_validation.sh \
  ...
  [[ ! -e "$SUMMARY" && ! -e "$OUTPUT/config.json" && ! -e "$OUTPUT/b2_specgr" ]] \
    || { STAGE=finished; write_status failed "Refusing to overwrite existing S15-4 B2 artifacts." 10; exit 10; }
  ...
  STAGE=s15_4_beauty_b2_full_validation
  write_status running "B0/B1/B2 frozen Beauty 10,655-event validation active on GPU${GPU}."
  timeout --signal=TERM --kill-after=30 "$HARD_TIMEOUT_SECONDS" \
    ...
    "$PYTHON" experiment/phase15/protocol/toys_s3b_full_validation.py \
      --experiment-id GRAM_STAGE15_S4_BEAUTY_FULL_VALIDATION_B0_B1_B2_SEED0 \
      --domain Beauty_cold50 --completed-verdict COMPLETED_S15_4_BEAUTY_B2_FULL_VALIDATION \
      --required-events 10655 --progress-marker s4-b2-eval \
      ...
      --candidate-chunk-size 10 --similar-top-k 10 \
      --bootstrap-resamples 10000 --bootstrap-seed 20260822 --seed 1502 &
  WORKLOAD_PID=$!
  write_status running "S15-4 B2 workload active; Beauty validation is evaluation-only, test remains sealed, and automatic retry is disabled."
  ...
  if (( rc != 0 )); then
    ...
    if (( rc == 124 || rc == 137 )); then
      write_status timeout "S15-4 B2 reached hard timeout; no automatic retry." "$rc"
    else
      write_status failed "S15-4 B2 exited rc=${rc}; no automatic retry." "$rc"
    fi
    exit "$rc"
  fi
  ...
  verdict=$("$PYTHON" -c 'import json,sys; print(json.load(open(sys.argv[1]))["verdict"])' "$SUMMARY") \
    || { STAGE=finished; write_status failed "Could not read S15-4 B2 verdict." 12; exit 12; }
  STAGE=finished
...
case "$ACTION" in
  start)
    ...
    STARTED_AT=$(date -Is)
    EXACT_START_COMMAND=${EXACT_START_COMMAND_OVERRIDE:-"bash experiment/phase15/run_stage15_s4_beauty_b2_full_validation.sh start $GPU"}
    STAGE=starting
    write_status starting "Background S15-4 Beauty B2 full validation starting on GPU${GPU}."
    printf -v launch 'env OUTPUT_OVERRIDE=%q SESSION_OVERRIDE=%q EXACT_START_COMMAND_OVERRIDE=%q bash %q worker %q %q %q >> %q 2>&1' "$OUTPUT" "$SESSION" "$EXACT_START_COMMAND" "$ROOT/experiment/phase15/run_stage15_s4_beauty_b2_full_validation.sh" "$GPU" "$STARTED_AT" "$EXACT_START_COMMAND" "$LOG"
