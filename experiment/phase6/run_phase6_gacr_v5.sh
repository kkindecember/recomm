...
SESSION=gram_phase6_gacr_v5
GPU=0
PYTHON=/home/jiangtangyunzhi/miniconda3/envs/gram-repro/bin/python3.9
WORKLOAD="$ROOT/experiment/phase6/gacr_v5.py"
...
write_status() {
  ...
  printf '{"experiment_id":"GRAM_PHASE6_GACR_V5","status":"%s","stage":"%s","reason":"%s","started_at":"%s","updated_at":"%s","runner_pid":%s,"workload_pid":%s,"tmux_session":"%s","physical_gpu":0,"log_path":"%s","result_path":"%s","resource_reservation":"%s","codellama_restore_gpu":0,"test_read":false,"sports_read":false}\n' \
...
finish() {
  ...
  if (( experiment_rc == 0 && restore_rc == 0 )); then
    write_status succeeded "GACR-v5 completed; results await researcher-requested analysis."
...
worker() {
  ...
  for required in "$CONFIG" "$WORKLOAD" \
    ...
    "$ROOT/artifacts/phase6/gacr_v4/Toys/gate_seed2023.pt" \
    "$ROOT/artifacts/phase6/gacr_v4/Toys/gate_seed2024.pt" \
    "$ROOT/artifacts/phase6/gacr_v4/Toys/gate_seed2025.pt" \
    "$ROOT/artifacts/phase6/gacr_v4/Beauty/gate_seed2023.pt" \
    "$ROOT/artifacts/phase6/gacr_v4/Beauty/gate_seed2024.pt" \
    "$ROOT/artifacts/phase6/gacr_v4/Beauty/gate_seed2025.pt"; do
    if [[ ! -s "$required" ]]; then
  ...
  write_status releasing_resource "Stopping CodeLlama before GACR-v5 on physical GPU0."
  reserver stop
...
main() {
  case "${1:-status}" in
    start)
      ...
      write_status starting "Persistent GACR-v5 session started for physical GPU0."
    ...
    status)
      ...
      ;;
    *)
      echo "usage: bash experiment/phase6/run_phase6_gacr_v5.sh {start|status|worker}" >&2
      exit 2
      ;;
  esac
}
