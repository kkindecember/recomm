...
CONFIG="$ROOT/artifacts/phase10/configs/cf1_a_toys_candidate_union_preregistered.json"
OUTPUT="$ROOT/artifacts/phase10/cf1_a_toys_candidate_union"
LOG="$OUTPUT/run.log"
STATUS="$OUTPUT/status.json"
SESSION=gram_phase10_cf1_a_candidate_union
PYTHON=/home/jiangtangyunzhi/miniconda3/envs/gram-repro/bin/python
EVALUATOR="$ROOT/experiment/phase10/eval_cf1_a_candidate_union.py"
TEST="$ROOT/experiment/phase10/test_cf1_a_candidate_union.py"
STARTED_AT=""
STAGE=not_started
WORKLOAD_PID=0
...
write_status() {
  ...
  mkdir -p "$OUTPUT"
  printf '{"experiment_id":"GRAM_PHASE10_CF1_A_TOYS_CANDIDATE_UNION_V1","status":"%s","stage":"%s","reason":"%s","started_at":"%s","updated_at":"%s","runner_pid":%s,"workload_pid":%s,"tmux_session":"%s","device":"cpu","test_read":false,"sports_read":false}\n' \
...
assert config["decision_status"] == "AUTHORIZED_CF1_A_CANDIDATE_UNION_COVERAGE"
...
finish() {
  ...
  if (( rc == 0 )); then
    ...
    write_status completed "CF1-A engineering completed; scientific gate=$gate."
  else
    write_status failed "CF1-A engineering exit=$rc; no automatic retry."
  fi
  exit "$rc"
}
...
worker() {
  ...
  trap 'exit 129' HUP
  STAGE=preflight
  write_status preflight "Verifying frozen validation inputs, code and tests."
  verify_config_and_locks
  "$PYTHON" -m pytest -q "$TEST"
  "$PYTHON" -m py_compile "$EVALUATOR"
  bash -n "$0"
  STAGE=full_catalog_candidate_union
  write_status running "CF1-A full-catalog candidate-union coverage running."
  timeout --signal=TERM 1800 "$PYTHON" "$EVALUATOR" \
    --output-dir "$OUTPUT" --batch-size 256 &
  WORKLOAD_PID=$!
  wait "$WORKLOAD_PID"
  WORKLOAD_PID=0
  [[ -s "$OUTPUT/summary.json" && -s "$OUTPUT/per_user_coverage.tsv" ]]
}
...
case "${1:-status}" in
  start)
    ...
    STAGE=starting
    write_status starting "Persistent CF1-A CPU session started."
