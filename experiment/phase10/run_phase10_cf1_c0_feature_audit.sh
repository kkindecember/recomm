...
CONFIG="$ROOT/artifacts/phase10/configs/cf1_c0_toys_feature_audit_preregistered.json"
OUTPUT="$ROOT/artifacts/phase10/cf1_c0_toys_feature_audit"
LOG="$OUTPUT/run.log"
STATUS="$OUTPUT/status.json"
SESSION=gram_phase10_cf1_c0_feature_audit
PYTHON=/home/jiangtangyunzhi/miniconda3/envs/gram-repro/bin/python
EVALUATOR="$ROOT/experiment/phase10/eval_cf1_c0_feature_audit.py"
TEST="$ROOT/experiment/phase10/test_cf1_c0_feature_audit.py"
STARTED_AT=""
STAGE=not_started
...
write_status() {
  ...
  mkdir -p "$OUTPUT"
  printf '{"experiment_id":"GRAM_PHASE10_CF1_C0_TOYS_FEATURE_AUDIT_V1","status":"%s","stage":"%s","reason":"%s","started_at":"%s","updated_at":"%s","runner_pid":%s,"tmux_session":"%s","test_read":false,"beauty_read":false,"sports_read":false}\n' \
...
assert config["decision_status"] == "AUTHORIZED_CF1_C0_FEATURE_AUDIT_ONLY"
...
finish() {
  ...
  STAGE=finished
  if (( rc == 0 )) && [[ -s "$OUTPUT/summary.json" && -s "$OUTPUT/feature_table.npz" ]]; then
    local gate
    gate=$("$PYTHON" -c 'import json,sys; print(json.load(open(sys.argv[1]))["scientific_gate"]["status"])' "$OUTPUT/summary.json")
    write_status completed "CF1-C0 completed; scientific gate=$gate."
  else
    write_status failed "CF1-C0 exit=$rc; no automatic retry."
  fi
  exit "$rc"
}
...
worker() {
  ...
  STAGE=feature_audit
  write_status running "Building full CF1-C0 feature table and frozen baselines."
  timeout --signal=TERM 3600 "$PYTHON" "$EVALUATOR" --output-dir "$OUTPUT" --batch-size 256
}
...
case "${1:-status}" in
  start)
    ...
    STAGE=starting
    write_status starting "Persistent CF1-C0 session started."
