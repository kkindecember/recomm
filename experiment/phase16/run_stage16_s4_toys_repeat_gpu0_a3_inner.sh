...
FORMAL_STATUS="$HOST_ROOT/artifacts/phase16/s4_toys_standalone/formal/toys_seed1502_gpu0_a3/status.json"
STATUS_ROOT="$HOST_ROOT/.runtime/phase16_s4_gpu0_repeat"
...
write_status() {
  ...
  mkdir -p "$STATUS_ROOT"
  printf '{"experiment_id":"GRAM_PHASE16_S4_GPU0_DISCARD_ONLY_REPEAT","status":"%s","status_code":"%s","stage":"%s","reason":"%s","started_at":"%s","updated_at":"%s","runner_pid":%d,"workload_pid":%d,"process_alive":%s,"physical_gpu":0,"visible_gpu":0,"minimum_free_mib":22528,"memory_only_admission":true,"attempt":%d,"completed_cycles":%d,"current_arm":"%s","discard_output":true,"repeat_artifacts_saved":false,"formal_output_read_only":true,"affects_scientific_results":false,"promotion_eligible":false,"planned_independent_repetitions":true,"automatic_retry_of_formal":false,"yield_to_new_gpu0_process":true,"existing_processes_modified":false,"test_read":false,"exit_code":%d}\n' \
...
if ! "$PYTHON" -m experiment.phase16.protocol.prepare_stage16_s4_gpu0_a3_runtime verify \
...
BASELINE_PIDS=$(gpu0_compute_pids | paste -sd' ' -)
write_status RUNNING WAITING_GPU0_MEMORY waiting_resources "Discard-only queue waits only for GPU0 free memory; it never writes predictions, metrics, checkpoints, or logs." true -1
...
while true; do
  while true; do
    ...
    write_status RUNNING WAITING_GPU0_MEMORY waiting_resources "Waiting only for GPU0 free memory or a new normal task to leave; utilization is ignored and no external PID is signaled." true -1
    sleep 5
  done
  ...
  for arm in S-AUX S-PLUS-CTRL S-PLUS G-RIDGE; do
    ...
    while kill -0 "$WORKLOAD_PID" 2>/dev/null; do
      ...
      if [[ -n "$foreign" ]]; then
        ...
        write_status RUNNING YIELDING_TO_NORMAL_WORK yielding "A new normal GPU0 task appeared; terminating only this discard workload." true -1
        terminate_own_workload
        break
      fi
  ...
  write_status RUNNING DISCARD_CYCLE_COMPLETED cycle_transition "One independent discard-only cycle completed; no result was saved and the next cycle starts when GPU0 has enough free memory." true 0
done
