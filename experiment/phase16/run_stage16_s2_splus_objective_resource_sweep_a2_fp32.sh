...
CONFIG=experiment/phase16/configs/stage16_s2_splus_objective_resource_sweep_a2_fp32.json
OUTPUT="$ROOT/artifacts/phase16/s2_splus_objective_resource_sweep/gpu5_a2_fp32"
...
write_status() {
  ...
  mkdir -p "$OUTPUT"
  printf '{"experiment_id":"GRAM_PHASE16_S2_SPLUS_OBJECTIVE_RESOURCE_SWEEP","attempt_id":"s16_s2_splus_resource_gpu5_a2_fp32","status":"%s","status_code":"%s","reason":"%s","started_at":"%s","updated_at":"%s","physical_gpu":%d,"visible_gpu":0,"minimum_free_mib":10240,"admission_free_mib":%d,"hard_timeout_seconds":600,"holder_released":false,"workload_rc":%d,"exit_code":%d,"test_read":false,"automatic_retry":false,"exact_start_command":"bash experiment/phase16/run_stage16_s2_splus_objective_resource_sweep_a2_fp32.sh 5","output_dir":"artifacts/phase16/s2_splus_objective_resource_sweep/gpu5_a2_fp32","summary_path":"artifacts/phase16/s2_splus_objective_resource_sweep/gpu5_a2_fp32/summary.json","log_path":"artifacts/phase16/s2_splus_objective_resource_sweep/gpu5_a2_fp32/run.log"}\n' "$state" "$code" "$reason" "$STARTED_AT" "$(date -Is)" "$GPU" "$ADMISSION_FREE" "$rc" "$rc" > "$temporary"
  mv "$temporary" "$STATUS"
}
...
write_status running RUNNING "FP32 objective-complete S-PLUS/CTRL resource sweep running; no efficacy metric." -1
...
write_status completed COMPLETED "PASS_S16_2_SPLUS_OBJECTIVE_RESOURCE_SWEEP_FP32" 0
