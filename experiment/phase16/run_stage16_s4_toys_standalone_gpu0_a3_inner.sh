...
OUTPUT_REL=artifacts/phase16/s4_toys_standalone/formal/toys_seed1502_gpu0_a3
...
HEARTBEAT=30
EXACT_COMMAND="bash experiment/phase16/run_stage16_s4_toys_standalone_gpu0_a3.sh"
ATTEMPT_ID=s16_s4_toys_standalone_gpu0_a3
EXPERIMENT_ID=GRAM_PHASE16_S4_TOYS_STANDALONE_FROZEN_VALIDATION
ISOLATED_RUNTIME_ROOT=.runtime/phase16_s4_toys_gpu0_a3_runtime
REPEAT_SESSION=phase16_s4_toys_repeat_gpu0_a3
REPEAT_INNER=experiment/phase16/run_stage16_s4_toys_repeat_gpu0_a3_inner.sh
HOST_ROOT=$(dirname "$(readlink -f "$ROOT/artifacts")")
REPEAT_STATUS_REL=.runtime/phase16_s4_gpu0_repeat/status.json
...
LAST_REASON="S16-4 GPU0 a3 has not reached a controlled terminal state."
LAST_RC=1
COMPUTE_DEADLINE=0
...
handle_signal() {
  terminate_own_workload
  stop_telemetry
  terminal_status FAILED INTERRUPTED "Formal S16-4 GPU0 runner received a signal; partial a3 data remains isolated and no retry was started." 143
  exit 143
}
...
handle_exit() {
  ...
  if [[ "$TERMINAL_WRITTEN" != true ]]; then
    terminal_status FAILED RUNNER_EXIT "Formal S16-4 GPU0 runner exited without a controlled terminal state; no retry was started." "$rc"
  fi
}
...
wait_for_gpu0_memory() {
  STAGE=waiting_gpu0_memory
  while true; do
    ...
    write_status RUNNING WAITING_GPU0_MEMORY "Waiting only for GPU0 free memory to reach 22528 MiB; utilization is not an admission condition and no existing process is modified." -1 false true
    sleep 5
  done
}
...
run_gpu_step() {
  ...
  if [[ "$mode" == smoke ]]; then
    ...
    write_status RUNNING RUNNING "Running one discard-only bounded resource-smoke event for $arm on GPU0." -1 true true
    ...
    STAGE=formal_arm
    write_status RUNNING RUNNING "Running the write-once formal $arm validation arm on GPU0." -1 true true
    env CUDA_VISIBLE_DEVICES="$GPU" CUBLAS_WORKSPACE_CONFIG=:4096:8 \
      timeout --signal=TERM --kill-after=20 "$timeout_seconds" \
      "$PYTHON" -m "$VALIDATOR_MODULE" --config "$CONFIG" --arm "$arm" \
  ...
  while kill -0 "$WORKLOAD_PID" 2>/dev/null; do
    sleep "$HEARTBEAT"
    write_status RUNNING RUNNING "S16-4 GPU0 $mode step for $arm is active; existing processes have not been modified." -1 true true
  done
...
if [[ -e "$OUTPUT" || -L "$OUTPUT" ]]; then
  echo "Refusing existing formal S16-4 GPU0 artifact root." >&2
  exit 8
fi
mkdir -p "$OUTPUT"
trap handle_signal TERM INT HUP
trap handle_exit EXIT
write_status RUNNING RUNNING "Verifying isolated runtime and disk before memory-only GPU0 admission." -1 false true
...
rc=$?
if (( rc != 0 )); then
  terminal_status FAILED RUNTIME_IDENTITY_FAILED "The immutable S16-4 GPU0 runtime failed verification; no GPU code started." "$rc"
  exit "$rc"
fi
...
for arm in S-AUX S-PLUS-CTRL S-PLUS G-RIDGE; do
  ...
  if (( rc != 0 )); then
    ...
    (( rc == 124 )) && code=TIMEOUT
    terminal_status FAILED "$code" "The write-once formal $arm arm failed; partial a3 artifacts are retained and no retry was started." "$rc"
    exit "$rc"
  fi
  PROGRESS=$((PROGRESS + 1))
done
...
PROGRESS=9
terminal_status COMPLETED COMPLETED_S16_4_TOYS_STANDALONE_FROZEN_VALIDATION "S16-4 Toys standalone validation completed on GPU0; all formal outputs are sealed and repeat compute remains non-promotional." 0
...
if [[ "$REPEAT_STARTED" == true ]]; then
  write_status "$LAST_STATE" "$LAST_CODE" "S16-4 formal outputs are sealed; the GPU0 discard-only repeat queue has started and cannot write scientific artifacts." "$LAST_RC" false false
else
