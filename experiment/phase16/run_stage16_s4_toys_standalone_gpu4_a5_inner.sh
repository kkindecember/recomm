...
CONFIG=experiment/phase16/configs/stage16_s4_toys_standalone_gpu4_a5.json
...
OUTPUT_REL=artifacts/phase16/s4_toys_standalone/formal/toys_seed1502_gpu4_a5
...
HEARTBEAT=30
EXACT_COMMAND="bash experiment/phase16/run_stage16_s4_toys_standalone_gpu4_a5.sh"
ATTEMPT_ID=s16_s4_toys_standalone_gpu4_a5
EXPERIMENT_ID=GRAM_PHASE16_S4_TOYS_STANDALONE_FROZEN_VALIDATION
ISOLATED_RUNTIME_ROOT=.runtime/phase16_s4_toys_gpu4_a5_runtime
REPEAT_SESSION=phase16_s4_toys_repeat_gpu4_a5
REPEAT_INNER=experiment/phase16/run_stage16_s4_toys_repeat_gpu4_a5_inner.sh
...
LAST_REASON="S16-4 GPU4 a5 has not reached a controlled terminal state."
LAST_RC=1
COMPUTE_DEADLINE=0
...
handle_signal() {
  terminate_own_workload
  stop_telemetry
  terminal_status FAILED INTERRUPTED "Formal S16-4 GPU4 runner received a signal; partial a5 data remains isolated and no retry was started." 143
  exit 143
}
...
timeout --signal=TERM --kill-after=10 600 \
  "$PYTHON" -m experiment.phase16.protocol.prepare_stage16_s4_gpu4_a5_runtime \
...
for arm in S-AUX S-PLUS-CTRL S-PLUS G-RIDGE; do
  ...
  if (( rc != 0 )); then
    ...
    (( rc == 124 )) && code=TIMEOUT
    terminal_status FAILED "$code" "The write-once formal $arm arm failed; partial a5 artifacts are retained and no retry was started." "$rc"
    exit "$rc"
  fi
  PROGRESS=$((PROGRESS + 1))
done
