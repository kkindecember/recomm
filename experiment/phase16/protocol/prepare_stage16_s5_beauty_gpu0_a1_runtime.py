#!/usr/bin/env python3
"""Build and verify the immutable S16-5 Beauty S-AUX GPU0 runtime."""
...
DEFAULT_SNAPSHOT = ROOT / ".runtime/phase16_s5_beauty_saux_gpu0_a1_runtime"
CONFIG = "experiment/phase16/configs/stage16_s5_beauty_saux_gpu0_a1.json"
EXPECTED_SNAPSHOT_REL = ".runtime/phase16_s5_beauty_saux_gpu0_a1_runtime"
FORMAL_OUTPUT_WRITE_SCOPE = "artifacts/phase16/s5_beauty_saux/formal/beauty_seed1502_gpu0_a1"
OVERLAYS = (
    CONFIG,
    "experiment/phase13/protocol/b1_portfolio_confirmation.py",
    "experiment/phase16/configs/stage16_s5_beauty_saux_train_gpu0_a1.json",
    "experiment/phase16/configs/stage16_s4_toys_standalone_gpu4_a7.json",
    ...
    "experiment/phase16/tests/test_stage16_s5_beauty_saux.py",
    "experiment/phase16/run_stage16_s5_beauty_saux_gpu0_a1.sh",
    "experiment/phase16/run_stage16_s5_beauty_saux_gpu0_a1_inner.sh",
)
...
def identity_paths(snapshot: Path) -> list[Path]:
    roots = (
        snapshot / "experiment/phase13/protocol",
...
def verify_snapshot(snapshot: Path) -> dict[str, Any]:
    ...
    if manifest_path.is_symlink() or not manifest_path.is_file():
        raise ValueError("Missing S16-5 isolated-runtime manifest")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if snapshot.resolve() != Path(manifest["snapshot_root"]).resolve():
        raise ValueError("S16-5 isolated-runtime root drift")
    for relative, target in (
        ...
        if not link.is_symlink() or link.resolve() != target.resolve():
            raise ValueError(f"S16-5 shared parent link drift: {relative}")
    for relative, expected in manifest["code_sha256"].items():
        path = snapshot / relative
        if path.is_symlink() or not path.is_file() or sha256(path) != expected:
            raise ValueError(f"S16-5 isolated code drift: {relative}")
    config_path = snapshot / CONFIG
    if sha256(config_path) != manifest["config_sha256"]:
        raise ValueError("S16-5 isolated config drift")
    config = json.loads(config_path.read_text(encoding="utf-8"))
    if (
        config.get("physical_gpu") != 0
        or config.get("resources", {}).get("minimum_free_mib") != 9216
        or config.get("runtime_isolation", {}).get("snapshot_root") != EXPECTED_SNAPSHOT_REL
        or config.get("test_read") is not False
        or config.get("automatic_retry") is not False
    ):
        raise ValueError("S16-5 GPU0 isolation contract drift")
    return manifest
...
def prepare_snapshot(snapshot: Path) -> dict[str, Any]:
    ...
    if temporary.exists() or temporary.is_symlink():
        raise FileExistsError(f"Stale S16-5 runtime temporary exists: {temporary}")
    try:
        shutil.copytree(
            SOURCE_RUNTIME,
            temporary,
        ...
        for relative in OVERLAYS:
            ...
            if source.is_symlink() or not source.is_file():
                raise ValueError(f"Missing/non-regular S16-5 overlay: {relative}")
            destination.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(source, destination)
        code_sha: dict[str, str] = {}
        for path in identity_paths(temporary):
            if path.is_symlink() or not path.is_file():
                raise ValueError(f"Missing/non-regular S16-5 identity file: {path}")
        ...
        manifest = {
            "schema_version": "stage16_s5_beauty_saux_gpu0_a1_isolated_runtime_v1",
            ...
            "formal_output_write_scope": FORMAL_OUTPUT_WRITE_SCOPE,
            "test_read": False,
