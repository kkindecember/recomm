...
CONFIG=experiment/phase16/configs/stage16_s3r_gridge_formal_admission_gpu5_f1.json
ATTEMPT_ID=s16_s3r_gridge_formal_gpu5_f1
EXPERIMENT_ID=GRAM_PHASE16_S3R_GRIDGE_FORMAL_ADMISSION
OUTPUT_REL=artifacts/phase16/s3_genrecedit/inspired_ridge/admission/toys_seed1502_gpu5_f1
...
HARD_TIMEOUT=604800
STALL_ADVISORY=3600
EXACT_COMMAND="bash experiment/phase16/run_stage16_s3r_gridge_formal_admission_gpu5_f1.sh"
STARTED_AT=$(date -Is)
WORKLOAD_PID=0
TELEMETRY_PID=0
ADMISSION_FREE=0
...
mkdir -p "${OUTPUT%/*}"
BOOT_LOG="${OUTPUT%/*}/.formal_gpu5_f1_boot.log.$$"
...
TOTAL=1
PROGRESS_UNIT=formal_finalization
FORMAL_REASON="S16-3 authoritative experiment is finished: all 302400 requests, six FP64 G-RIDGE solves, 7435 item-disjoint events, and 512 warm-preservation pairs completed; the formal contract Gate passed. Admission metrics are recorded as non-promotional and validation/test stayed sealed."
terminal_status COMPLETED PASS_S16_3R_GRIDGE_CONTRACT_ADMISSION "$FORMAL_REASON Stability queue launch is now being attempted from the completed state." 0
if bash experiment/phase16/run_stage16_s3r_gridge_stability_gpu5.sh >> "$LOG" 2>&1; then
  STABILITY_QUEUE_STARTED=true
  write_status COMPLETED PASS_S16_3R_GRIDGE_CONTRACT_ADMISSION "$FORMAL_REASON The isolated full-compute stability queue has started; its outputs cannot affect the authoritative result." 0 false false
else
  write_status COMPLETED PASS_S16_3R_GRIDGE_CONTRACT_ADMISSION "$FORMAL_REASON Stability queue launch failed, but the authoritative completed result remains valid and unchanged." 0 false false
fi
exit 0
