...
GPU=3
MIN_FREE_MIB=30720
SESSION=gram_phase3_cpbd_g0_d2
...
STATUS="$ROOT/experiment/phase3/phase3_cpbd_g0_d2_status.json"
PID_FILE="$ROOT/experiment/phase3/phase3_cpbd_g0_d2.pid"
LOG="$ROOT/artifacts/phase3/logs/cpbd_g0_d2.log"
BOARD_LOG="$ROOT/experiment/phase3/cpbd_g0_d2_gpu_board.csv"
PROCESS_LOG="$ROOT/experiment/phase3/cpbd_g0_d2_gpu_process.csv"
DISK_LOG="$ROOT/experiment/phase3/cpbd_g0_d2_disk.csv"
started_at=$(date -Is)
workload_pid=""
monitor_pid=""
...
write_status() {
    ...
    printf '{\n  "experiment": "GRAM phase3 CPBD G0-D2",\n  "updated_at": "%s",\n  "started_at": "%s",\n  "status": "%s",\n  "reason": "%s",\n  "gpu_selected": 3,\n  "runner_pid": %s,\n  "workload_pid": %s,\n  "log": "artifacts/phase3/logs/cpbd_g0_d2.log",\n  "output": "artifacts/phase3/cpbd_g0_d2/summary.json",\n  "resource_reservation": "%s"\n}\n' \
...
restore_resource() {
    ...
    write_status restoring_resource "CPBD G0-D2 ended; restoring CodeLlama on physical GPU3."
...
finish() {
    ...
    if (( experiment_rc == 0 && restore_rc == 0 )); then
        write_status succeeded "CPBD G0-D2 completed; inspect summary.json."
    elif (( restore_rc != 0 )); then
        write_status failed_to_restore_resource "G0-D2 exit=${experiment_rc}; resource restore failed."
    else
        write_status failed "G0-D2 exit=${experiment_rc}; no scientific decision was inferred."
    fi
    rm -f "$PID_FILE"
...
worker() {
    ...
    monitor_resources &
    monitor_pid=$!
    write_status running "Frozen fixed-budget CPBD scoring is running; no training, beam generation, or test access."
    CUDA_VISIBLE_DEVICES="$GPU" \
        HF_HOME="$ROOT/.cache/huggingface" \
        TRANSFORMERS_CACHE="$ROOT/.cache/huggingface" \
        "$PYTHON" "$ROOT/experiment/phase3/cpbd_g0_d2.py" \
        --config "$ROOT/artifacts/phase3/configs/cpbd_g0_d2_preregistered.json" &
    workload_pid=$!
    write_status running "Frozen fixed-budget CPBD scoring is running; no training, beam generation, or test access."
    wait "$workload_pid"
    workload_pid=""
...
case "$ACTION" in
    start)
        ...
        tmux new-session -d -s "$SESSION" \
            "bash '$ROOT/experiment/phase3/run_phase3_cpbd_g0_d2.sh' worker '$started_at'"
