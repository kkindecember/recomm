...
STATUS="$ROOT/experiment/phase3/phase3_pens_h0_d_status.json"
PID_FILE="$ROOT/experiment/phase3/phase3_pens_h0_d.pid"
LOG="$ROOT/artifacts/phase3/logs/pens_h0_d.log"
BOARD_LOG="$ROOT/experiment/phase3/pens_h0_d_gpu_board.csv"
PROCESS_LOG="$ROOT/experiment/phase3/pens_h0_d_gpu_process.csv"
DISK_LOG="$ROOT/experiment/phase3/pens_h0_d_disk.csv"
started_at=$(date -Is)
workload_pid=""
monitor_pid=""
...
write_status() {
    ...
    printf '{\n  "experiment": "GRAM phase3 PENS H0-D",\n  "updated_at": "%s",\n  "started_at": "%s",\n  "status": "%s",\n  "reason": "%s",\n  "gpu_selected": 3,\n  "runner_pid": %s,\n  "workload_pid": %s,\n  "log": "artifacts/phase3/logs/pens_h0_d.log",\n  "output": "artifacts/phase3/pens_h0_d/summary.json",\n  "resource_reservation": "%s"\n}\n' \
...
restore_resource() {
    ...
    write_status restoring_resource "PENS H0-D ended; restoring CodeLlama on physical GPU3."
...
finish() {
    ...
    if (( experiment_rc == 0 && restore_rc == 0 )); then
        write_status succeeded "PENS H0-D completed; inspect summary.json."
    elif (( restore_rc != 0 )); then
        write_status failed_to_restore_resource "H0-D exit=${experiment_rc}; resource restore failed."
    else
        write_status failed "H0-D exit=${experiment_rc}; no scientific decision was inferred."
    fi
    rm -f "$PID_FILE"
    ...
    while true; do
        ...
        if (( tick % 60 == 0 )); then
            df -Pk "$ROOT" | awk -v ts="$timestamp" \
                'NR==2 {print ts","$1","$2","$3","$4","$5","$6}' >> "$DISK_LOG" || true
        fi
        tick=$((tick + 1))
        sleep 5
    done
...
worker() {
    ...
    monitor_resources &
    monitor_pid=$!
    write_status running "Frozen PENS norm-only scoring is running; no training, beam generation, or test access."
    CUDA_VISIBLE_DEVICES="$GPU" \
        HF_HOME="$ROOT/.cache/huggingface" \
        TRANSFORMERS_CACHE="$ROOT/.cache/huggingface" \
        "$PYTHON" "$ROOT/experiment/phase3/pens_h0_d.py" \
        --config "$ROOT/artifacts/phase3/configs/pens_h0_d_preregistered.json" &
    workload_pid=$!
    write_status running "Frozen PENS norm-only scoring is running; no training, beam generation, or test access."
    wait "$workload_pid"
    workload_pid=""
...
case "$ACTION" in
    start)
        ...
        tmux new-session -d -s "$SESSION" \
            "bash '$ROOT/experiment/phase3/run_phase3_pens_h0_d.sh' worker '$started_at'"
