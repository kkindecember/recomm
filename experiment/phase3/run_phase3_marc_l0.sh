...
STATUS="$ROOT/experiment/phase3/phase3_marc_l0_status.json"
PID_FILE="$ROOT/experiment/phase3/phase3_marc_l0.pid"
LOG="$ROOT/artifacts/phase3/logs/marc_l0.log"
BOARD_LOG="$ROOT/experiment/phase3/marc_l0_gpu_board.csv"
PROCESS_LOG="$ROOT/experiment/phase3/marc_l0_gpu_process.csv"
started_at=$(date -Is)
workload_pid=""
monitor_pid=""
...
write_status() {
    ...
    printf '{\n  "experiment": "GRAM phase3 MARC L0",\n  "updated_at": "%s",\n  "started_at": "%s",\n  "status": "%s",\n  "reason": "%s",\n  "gpu_selected": 3,\n  "runner_pid": %s,\n  "workload_pid": %s,\n  "log": "artifacts/phase3/logs/marc_l0.log",\n  "output": "artifacts/phase3/marc_l0/summary.json",\n  "resource_reservation": "%s"\n}\n' \
...
restore_resource() {
    ...
    write_status restoring_resource "MARC L0 ended; restoring CodeLlama on physical GPU3."
...
finish() {
    ...
    if (( experiment_rc == 0 && restore_rc == 0 )); then
        write_status succeeded "MARC L0 completed; inspect summary.json."
    elif (( restore_rc != 0 )); then
        write_status failed_to_restore_resource "L0 exit=${experiment_rc}; resource restore failed."
    else
        write_status failed "L0 exit=${experiment_rc}; no scientific decision inferred."
    fi
    rm -f "$PID_FILE"
...
worker() {
    ...
    monitor_resources &
    monitor_pid=$!
    write_status running "Training-only frozen counterfactual scoring and tiny-critic audit are running."
    CUDA_VISIBLE_DEVICES="$GPU" \
        HF_HOME="$ROOT/.cache/huggingface" \
        TRANSFORMERS_CACHE="$ROOT/.cache/huggingface" \
        "$PYTHON" "$ROOT/experiment/phase3/marc_l0.py" \
        --config "$ROOT/artifacts/phase3/configs/marc_l0_preregistered.json" &
    workload_pid=$!
    write_status running "Training-only frozen counterfactual scoring and tiny-critic audit are running."
    wait "$workload_pid"
    workload_pid=""
...
case "$ACTION" in
    start)
        ...
        tmux new-session -d -s "$SESSION" \
            "bash '$ROOT/experiment/phase3/run_phase3_marc_l0.sh' worker '$started_at'"
