...
GPU=3
MIN_FREE_MIB=30720
SESSION=gram_phase3_cgi_e0
RESERVER="$ROOT/tools/run_codellama.sh"
PYTHON=/home/jiangtangyunzhi/miniconda3/envs/gram-repro/bin/python3.9
STATUS="$ROOT/experiment/phase3/phase3_cgi_e0_status.json"
PID_FILE="$ROOT/experiment/phase3/phase3_cgi_e0.pid"
LOG="$ROOT/artifacts/phase3/logs/cgi_e0.log"
BOARD_LOG="$ROOT/experiment/phase3/cgi_e0_gpu_board.csv"
PROCESS_LOG="$ROOT/experiment/phase3/cgi_e0_gpu_process.csv"
started_at=$(date -Is)
workload_pid=""
monitor_pid=""
...
write_status() {
    ...
    printf '{\n  "experiment": "GRAM phase3 CGI E0-D",\n  "updated_at": "%s",\n  "started_at": "%s",\n  "status": "%s",\n  "reason": "%s",\n  "gpu_selected": 3,\n  "runner_pid": %s,\n  "workload_pid": %s,\n  "log": "artifacts/phase3/logs/cgi_e0.log",\n  "output": "artifacts/phase3/cgi_e0/summary.json",\n  "resource_reservation": "%s"\n}\n' \
...
restore_resource() {
    ...
    write_status restoring_resource "CGI E0-D ended; restoring CodeLlama on physical GPU3."
...
finish() {
    ...
    if (( experiment_rc == 0 && restore_rc == 0 )); then
        write_status succeeded "CGI E0-D completed; inspect the locked decision in summary.json."
    elif (( restore_rc != 0 )); then
        write_status failed_to_restore_resource "E0-D exit=${experiment_rc}; resource restore failed."
    else
        write_status failed "E0-D exit=${experiment_rc}; no scientific decision was inferred."
    fi
    rm -f "$PID_FILE"
...
worker() {
    ...
    monitor_resources &
    monitor_pid=$!
    write_status running "Frozen Toys/Beauty counterfactual scoring is running; no training or test access."
    CUDA_VISIBLE_DEVICES="$GPU" \
        HF_HOME="$ROOT/.cache/huggingface" \
        TRANSFORMERS_CACHE="$ROOT/.cache/huggingface" \
        "$PYTHON" "$ROOT/experiment/phase3/cgi_e0.py" \
        --config "$ROOT/artifacts/phase3/configs/cgi_e0_preregistered.json" &
    workload_pid=$!
    write_status running "Frozen Toys/Beauty counterfactual scoring is running; no training or test access."
    wait "$workload_pid"
    workload_pid=""
...
case "$ACTION" in
    start)
        ...
        tmux new-session -d -s "$SESSION" \
            "bash '$ROOT/experiment/phase3/run_phase3_cgi_e0.sh' worker '$started_at'"
