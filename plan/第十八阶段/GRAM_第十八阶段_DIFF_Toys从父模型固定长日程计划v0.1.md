...
**2026-09-10约17:35核查与建议：** 当前第16/21轮，已完成15轮，任务仍运行，尚无最终全量结果。固定2000人子集最佳仍是第1轮NDCG@10=0.069395956（相对同用户原GRAM+0.0366%）；最新第15轮为0.069075062（−0.4260%），第9轮后缓慢回升但尚未超过冻结阶段或原GRAM。Beauty固定日程最终仅+0.1791%，配对区间包含0。按寻找实际提点的当前目标，建议在完整checkpoint边界停止本版Toys后续长训，不再自动追加；**这是建议，尚未执行停止，原21轮日程仍有效**。若跑完，估计仍需约11–13小时；若用户决定中止，应保留模型/optimizer/RNG及日志，明确记为未完成全日程，不能宣称完整21轮方案已失败。详见[投入分析](../../report/第十八阶段/Stage18_DIFF_GRAM_继续投入价值与最小验证路径.md)与[核查快照](../../artifacts/phase18/diff_gram/continuation_review/review_20260910.json)。
