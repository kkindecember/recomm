...
> 状态：`S16_3F_RESOURCE_A3_TIMEOUT_PRESERVED / S16_3F_TERMINAL_LINEAR_SYSTEM_BLOCKED / S16_3B_B1_FAILED_PRESERVED / S16_3B_RECOVERY_C1_STRUCTURAL_BLOCKED_PASS / S16_3R_RESOURCE_R1_GPU5_FP32_CAST_BLOCKED_PRESERVED / S16_3R_RESOURCE_R2_FP64_SOLVE_PASS / S16_3R_FORMAL_F1_CODE_IDENTITY_FAILED_PRESERVED / S16_3R_FORMAL_F2_ISOLATED_PATH_FAILED_PRESERVED / S16_3R_F2_REPEAT_INTERRUPTED_FOR_FORMAL_PRIORITY / S16_3R_FORMAL_F3_GPU5_RUNNING`
>
> 当前 Gate：faithful 历史 Gate `NOT_PASSED / PROVEN_STRUCTURAL_RANK_BLOCKED_FAITHFUL_NO_RIDGE`；新方法资源 Gate `PASS_S16_3R_GRIDGE_OBJECTIVE_RESOURCE_SWEEP` 已通过，正式 Gate `PASS_S16_3R_GRIDGE_CONTRACT_ADMISSION` 为 `F3_RUNNING_PENDING`
>
...
- 当前裁决：resource Gate 保持 PASS；formal `PASS_S16_3R_GRIDGE_CONTRACT_ADMISSION` 仍未通过，S16-4 G-RIDGE arm 未解锁。任何新的 authoritative formal attempt 必须使用新 attempt root 并再次获得用户明确授权。
...
- f3 formal 运行期间没有 repeat 与其并发。只有 f3 到达任意 controlled terminal 后，才自动创建 `phase16_s3r_gridge_repeat_gpu5_f3`；其每轮使用独立 attempt/config/output，`affects_scientific_results=false / promotion_eligible=false / normal_experiment_priority=true`，检测到新的 GPU5 PID 时只终止自己的 cycle。当前 formal Gate 尚待 f3 终态，S16-4 仍锁定。
